		...
		<div class="row align-items-center justify-content-center mb-5">
			<div class="col">
				...
				<div class="container">
					...
					<?php } ?>
					<div class="row mt-2">
						<div class="col-7 text-muted text-center"><small><?php printf( esc_html__( 'Scanned %s ago', 'infinite-uploads' ), human_time_diff( $stats['files_finished'] ) ); ?> &dash; <a href="#" class="badge badge-primary" data-toggle="modal" data-target="#scan-modal"><span
										data-toggle="tooltip"
				...
				<div class="row justify-content-center">
					<div class="col-4 text-center">
						<?php if ( ! empty( $stats['sync_finished'] ) ) { //if sync is finished show enable button ?>
							<button class="btn text-nowrap btn-primary btn-lg btn-block" data-toggle="modal" data-target="#enable-modal"><span class="dashicons dashicons-cloud"></span> <?php esc_html_e( 'Sync Now', 'infinite-uploads' ); ?></button>
						<?php } elseif ( ! empty( $stats['compare_finished'] ) ) { ?>
							<button class="btn text-nowrap btn-primary btn-lg btn-block" data-toggle="modal" data-target="#upload-modal"><span class="dashicons dashicons-cloud"></span> <?php esc_html_e( 'Sync Now', 'infinite-uploads' ); ?></button>
						<?php } else { ?>
							<button class="btn text-nowrap btn-primary btn-lg btn-block" id="iup-sync-button" data-toggle="modal" data-target="#scan-remote-modal"><span class="dashicons dashicons-cloud"></span> <?php esc_html_e( 'Sync Now', 'infinite-uploads' ); ?></button>
						<?php } ?>
					</div>
				</div>
			</div>
