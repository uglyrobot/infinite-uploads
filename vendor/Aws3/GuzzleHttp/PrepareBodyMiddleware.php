...
class PrepareBodyMiddleware
{
    ...
    public function __invoke(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    /**
     * Add expect header
     *
     * @return void
     */
    private function addExpectHeader(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
