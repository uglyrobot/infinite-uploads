...
use UglyRobot\Infinite_Uploads\GuzzleHttp\HandlerStack;
...
class MockHandler implements \Countable
{
    ...
    {
        return \UglyRobot\Infinite_Uploads\GuzzleHttp\HandlerStack::create(new self($queue, $onFulfilled, $onRejected));
    }
    /**
    ...
    public function __invoke(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (isset($options['on_headers'])) {
            ...
            try {
                ...
                $response = new \UglyRobot\Infinite_Uploads\GuzzleHttp\Exception\RequestException($msg, $request, $response, $e);
            }
        }
        if (is_callable($response)) {
            $response = call_user_func($response, $request, $options);
        }
        $response = $response instanceof \Exception ? \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\rejection_for($response) : \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\promise_for($response);
    ...
    public function append()
    {
        foreach (func_get_args() as $value) {
            if ($value instanceof ResponseInterface || $value instanceof \Exception || $value instanceof PromiseInterface || is_callable($value)) {
                ...
                throw new \InvalidArgumentException('Expected a response or ' . 'exception. Found ' . \UglyRobot\Infinite_Uploads\GuzzleHttp\describe_type($value));
            }
        }
    }
    /**
    ...
    private function invokeStats(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, array $options, \UglyRobot\Infinite_Uploads\Psr\Http\Message\ResponseInterface $response = null, $reason = null)
    {
        if (isset($options['on_stats'])) {
            $transferTime = isset($options['transfer_time']) ? $options['transfer_time'] : 0;
            $stats = new \UglyRobot\Infinite_Uploads\GuzzleHttp\TransferStats($request, $response, $transferTime, $reason);
