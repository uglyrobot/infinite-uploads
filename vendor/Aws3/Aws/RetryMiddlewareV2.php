...
use UglyRobot\Infinite_Uploads\Aws\Retry\ConfigurationInterface;
use UglyRobot\Infinite_Uploads\Aws\Retry\QuotaManager;
use UglyRobot\Infinite_Uploads\Aws\Retry\RateLimiter;
...
class RetryMiddlewareV2
{
    ...
    public static function createDefaultDecider(\UglyRobot\Infinite_Uploads\Aws\Retry\QuotaManager $quotaManager, $maxAttempts = 3, $options = [])
    {
        $retryCurlErrors = [];
        if (extension_loaded('curl')) {
            $retryCurlErrors[CURLE_RECV_ERROR] = true;
        }
        return function ($attempts, \UglyRobot\Infinite_Uploads\Aws\CommandInterface $command, $result) use($options, $quotaManager, $retryCurlErrors, $maxAttempts) {
    ...
    public function __construct(\UglyRobot\Infinite_Uploads\Aws\Retry\ConfigurationInterface $config, callable $handler, $options = [])
    {
        $this->options = $options;
        $this->maxAttempts = $config->getMaxAttempts();
        $this->mode = $config->getMode();
        $this->nextHandler = $handler;
        $this->quotaManager = new \UglyRobot\Infinite_Uploads\Aws\Retry\QuotaManager();
        ...
        if ($this->mode === 'adaptive') {
            $this->rateLimiter = isset($options['rate_limiter']) ? $options['rate_limiter'] : new \UglyRobot\Infinite_Uploads\Aws\Retry\RateLimiter();
        }
    }
