...
use UglyRobot\Infinite_Uploads\GuzzleHttp\Event\EndEvent;
...
use UglyRobot\Infinite_Uploads\GuzzleHttp\Message\ResponseInterface as GuzzleResponse;
use UglyRobot\Infinite_Uploads\GuzzleHttp\Promise;
use UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\Response as Psr7Response;
use UglyRobot\Infinite_Uploads\GuzzleHttp\Stream\Stream;
...
class GuzzleHandler
{
    ...
    public function __invoke(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\Promise(function () use($guzzlePromise) {
            try {
                $guzzlePromise->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\UglyRobot\Infinite_Uploads\GuzzleHttp\Message\ResponseInterface $response) {
            ...
            if ($exception instanceof RequestException) {
                if (isset($options['sink'])) {
                    if (!$options['sink'] instanceof Psr7StreamInterface) {
                        $exception->getResponse()->setBody(\UglyRobot\Infinite_Uploads\GuzzleHttp\Stream\Stream::factory(file_get_contents($options['sink'])));
                    }
                }
            }
            // Reject with information about the error.
            return new \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\RejectedPromise($this->prepareErrorData($exception));
        });
    }
    private function createGuzzleRequest(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $psrRequest, array $options)
    {
        $ringConfig = [];
        ...
        // Prepare sink option.
        if (isset($options['sink'])) {
            $ringConfig['save_to'] = $options['sink'] instanceof Psr7StreamInterface ? new \UglyRobot\Infinite_Uploads\Aws\Handler\GuzzleV5\GuzzleStream($options['sink']) : $options['sink'];
            unset($options['sink']);
        }
        ...
        if (is_callable($statsCallback)) {
            $request->getEmitter()->on('end', function (\UglyRobot\Infinite_Uploads\GuzzleHttp\Event\EndEvent $event) use($statsCallback) {
        ...
        if ($body->getSize() === 0) {
            ...
            $request->setBody(new \UglyRobot\Infinite_Uploads\Aws\Handler\GuzzleV5\GuzzleStream($body));
        }
        $request->setHeaders($psrRequest->getHeaders());
        $request->setHeader('User-Agent', $request->getHeader('User-Agent') . ' ' . \UglyRobot\Infinite_Uploads\GuzzleHttp\Client::getDefaultUserAgent());
    ...
    private function createPsr7Response(\UglyRobot\Infinite_Uploads\GuzzleHttp\Message\ResponseInterface $response)
    {
        if ($body = $response->getBody()) {
            $body = new \UglyRobot\Infinite_Uploads\Aws\Handler\GuzzleV5\PsrStream($body);
        }
        return new \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders(), $body, $response->getReasonPhrase());
    }
