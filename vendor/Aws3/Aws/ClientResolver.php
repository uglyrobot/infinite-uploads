...
use UglyRobot\Infinite_Uploads\Aws\ClientSideMonitoring\Configuration;
use UglyRobot\Infinite_Uploads\Aws\Credentials\Credentials;
...
use UglyRobot\Infinite_Uploads\Aws\EndpointDiscovery\ConfigurationInterface;
use UglyRobot\Infinite_Uploads\Aws\EndpointDiscovery\ConfigurationProvider;
...
use UglyRobot\Infinite_Uploads\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use UglyRobot\Infinite_Uploads\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
...
use UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialProvider;
...
class ClientResolver
{
     ...
     */
    public function resolve(array $args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        $args['config'] = [];
    ...
    public static function _apply_retries($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        // A value of 0 for the config option disables retries
        if ($value) {
            $config = \UglyRobot\Infinite_Uploads\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                // # of retries is 1 less than # of attempts
                $decider = \UglyRobot\Infinite_Uploads\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1);
                $list->appendSign(\UglyRobot\Infinite_Uploads\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
            } else {
                $list->appendSign(\UglyRobot\Infinite_Uploads\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries']]), 'retry');
            }
        }
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            $args['credentials'] = \UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialProvider::fromCredentials($value);
        } elseif (is_array($value) && isset($value['key']) && isset($value['secret'])) {
            $args['credentials'] = \UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialProvider::fromCredentials(new \UglyRobot\Infinite_Uploads\Aws\Credentials\Credentials($value['key'], $value['secret'], isset($value['token']) ? $value['token'] : null, isset($value['expires']) ? $value['expires'] : null));
        } elseif ($value === false) {
            $args['credentials'] = \UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialProvider::fromCredentials(new \UglyRobot\Infinite_Uploads\Aws\Credentials\Credentials('', ''));
            $args['config']['signature_version'] = 'anonymous';
        } elseif ($value instanceof CacheInterface) {
            $args['credentials'] = \UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialProvider::defaultProvider($args);
        } else {
            throw new \InvalidArgumentException('Credentials must be an instance of ' . 'UglyRobot\\Infinite_Uploads\\Aws\\Credentials\\CredentialsInterface, an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    public static function _default_credential_provider(array $args)
    {
        return \UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialProvider::defaultProvider($args);
    }
    public static function _apply_csm($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        if ($value === false) {
            $value = new \UglyRobot\Infinite_Uploads\Aws\ClientSideMonitoring\Configuration(false, \UglyRobot\Infinite_Uploads\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \UglyRobot\Infinite_Uploads\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \UglyRobot\Infinite_Uploads\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
        $list->appendBuild(\UglyRobot\Infinite_Uploads\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallMonitoringMiddleware');
        $list->appendAttempt(\UglyRobot\Infinite_Uploads\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallAttemptMonitoringMiddleware');
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        $api = new \UglyRobot\Infinite_Uploads\Aws\Api\Service(\UglyRobot\Infinite_Uploads\Aws\Api\ApiProvider::resolve($value, 'api', $args['service'], $args['version']), $value);
        if (empty($args['config']['signing_name']) && isset($api['metadata']['signingName'])) {
            $args['config']['signing_name'] = $api['metadata']['signingName'];
        }
        $args['api'] = $api;
        $args['parser'] = \UglyRobot\Infinite_Uploads\Aws\Api\Service::createParser($api);
        $args['error_parser'] = \UglyRobot\Infinite_Uploads\Aws\Api\Service::createErrorParser($api->getProtocol(), $api);
    }
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            if (!self::isValidRegion($args['region'])) {
                throw new \UglyRobot\Infinite_Uploads\Aws\Exception\InvalidRegionException('Region must be a valid RFC' . ' host label.');
            }
            // Invoke the endpoint provider and throw if it does not resolve.
            $result = \UglyRobot\Infinite_Uploads\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme'], 'options' => self::getEndpointProviderOptions($args)]);
    ...
    {
        return \UglyRobot\Infinite_Uploads\Aws\EndpointDiscovery\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_serializer($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        $list->prependBuild(\UglyRobot\Infinite_Uploads\Aws\Middleware::requestBuilder($value), 'builder');
    }
    public static function _apply_debug($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        if ($value !== false) {
            $list->interpose(new \UglyRobot\Infinite_Uploads\Aws\TraceMiddleware($value === true ? [] : $value, $args['api']));
        }
    }
    public static function _apply_stats($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        ...
        if ($args['stats']['timer']) {
            $list->prependInit(\UglyRobot\Infinite_Uploads\Aws\Middleware::timer(), 'timer');
        }
    }
    public static function _apply_profile($_, array &$args)
    {
        $args['credentials'] = \UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialProvider::ini($args['profile']);
    }
    public static function _apply_validate($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        if ($value === false) {
            return;
        }
        $validator = $value === true ? new \UglyRobot\Infinite_Uploads\Aws\Api\Validator() : new \UglyRobot\Infinite_Uploads\Aws\Api\Validator($value);
        $list->appendValidate(\UglyRobot\Infinite_Uploads\Aws\Middleware::validation($args['api'], $validator), 'validation');
    }
    public static function _apply_handler($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        $list->setHandler($value);
    }
    public static function _default_handler(array &$args)
    {
        return new \UglyRobot\Infinite_Uploads\Aws\WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        $args['handler'] = new \UglyRobot\Infinite_Uploads\Aws\WrappedHttpHandler($value, $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_user_agent($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        if (!is_array($value)) {
            $value = [$value];
        }
        ...
        array_unshift($value, 'aws-sdk-php/' . \UglyRobot\Infinite_Uploads\Aws\Sdk::VERSION);
        $args['ua_append'] = $value;
        $list->appendBuild(static function (callable $handler) use($value) {
            return function (\UglyRobot\Infinite_Uploads\Aws\CommandInterface $command, \UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request) use($handler, $value) {
                return $handler($command, $request->withHeader('User-Agent', implode(' ', array_merge($value, $request->getHeader('User-Agent')))));
            };
        });
    }
    public static function _apply_endpoint($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
    ...
    public static function _apply_idempotency_auto_fill($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        $enabled = false;
        $generator = null;
        if (is_bool($value)) {
        ...
        if ($enabled) {
            $list->prependInit(\UglyRobot\Infinite_Uploads\Aws\IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    public static function _default_endpoint_provider(array $args)
    {
        $options = self::getEndpointProviderOptions($args);
        return \UglyRobot\Infinite_Uploads\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
    }
    public static function _default_serializer(array $args)
    {
        return \UglyRobot\Infinite_Uploads\Aws\Api\Service::createSerializer($args['api'], $args['endpoint']);
    }
    public static function _default_signature_provider()
    {
        return \UglyRobot\Infinite_Uploads\Aws\Signature\SignatureProvider::defaultProvider();
    }
    public static function _default_signature_version(array &$args)
    {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : call_user_func(\UglyRobot\Infinite_Uploads\Aws\Endpoint\PartitionEndpointProvider::defaultProvider(), ['service' => $args['service'], 'region' => $args['region']]);
    ...
    public static function _missing_version(array $args)
    {
        ...
        $versions = \UglyRobot\Infinite_Uploads\Aws\Api\ApiProvider::defaultProvider()->getVersions($service);
