...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'UglyRobot\\Infinite_Uploads\\Aws\\Exception\\AwsException', $collectStats = false)
    {
    ...
    public function __invoke(\UglyRobot\Infinite_Uploads\Aws\CommandInterface $command, \UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\UglyRobot\Infinite_Uploads\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\UglyRobot\Infinite_Uploads\Aws\CommandInterface $command, \UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, \UglyRobot\Infinite_Uploads\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \UglyRobot\Infinite_Uploads\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, \UglyRobot\Infinite_Uploads\Aws\CommandInterface $command, array $stats)
    {
