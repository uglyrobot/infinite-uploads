...
class EndpointDiscoveryMiddleware
{
    ...
    public function __construct(callable $handler, \UglyRobot\Infinite_Uploads\Aws\AwsClient $client, array $args, $config)
    {
    ...
    public function __invoke(\UglyRobot\Infinite_Uploads\Aws\CommandInterface $cmd, \UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (isset($op['endpointdiscovery'])) {
            $config = \UglyRobot\Infinite_Uploads\Aws\EndpointDiscovery\ConfigurationProvider::unwrap($this->config);
            $isRequired = !empty($op['endpointdiscovery']['required']);
            if ($isRequired && !$config->isEnabled()) {
                throw new \UglyRobot\Infinite_Uploads\Aws\Exception\UnresolvedEndpointException('This operation ' . 'requires the use of endpoint discovery, but this has ' . 'been disabled in the configuration. Enable endpoint ' . 'discovery or use a different operation.');
            }
            // Continue only if enabled by config
            if ($config->isEnabled()) {
                if (isset($op['endpointoperation'])) {
                    throw new \UglyRobot\Infinite_Uploads\Aws\Exception\UnresolvedEndpointException('This operation is ' . 'contradictorily marked both as using endpoint discovery ' . 'and being the endpoint discovery operation. Please ' . 'verify the accuracy of your model files.');
                }
                ...
                // Check/create cache
                if (!isset(self::$cache)) {
                    self::$cache = new \UglyRobot\Infinite_Uploads\Aws\LruArrayCache($config->getCacheLimit());
                }
                if (empty($endpointList = self::$cache->get($cacheKey))) {
                    $endpointList = new \UglyRobot\Infinite_Uploads\Aws\EndpointDiscovery\EndpointList([]);
                }
    ...
    private function discoverEndpoint($cacheKey, \UglyRobot\Infinite_Uploads\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (isset($result['Endpoints'])) {
            ...
            $endpointList = new \UglyRobot\Infinite_Uploads\Aws\EndpointDiscovery\EndpointList($endpointData);
            self::$cache->set($cacheKey, $endpointList);
            return $endpointList->getEndpoint();
        }
        throw new \UglyRobot\Infinite_Uploads\Aws\Exception\UnresolvedEndpointException('The endpoint discovery operation ' . 'yielded a response that did not contain properly formatted ' . 'endpoint data.');
    }
    private function getCacheKey(\UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialsInterface $creds, \UglyRobot\Infinite_Uploads\Aws\CommandInterface $cmd, array $identifiers)
    {
    ...
    private function getDiscoveryCommand(\UglyRobot\Infinite_Uploads\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (!isset($endpointOperation)) {
            throw new \UglyRobot\Infinite_Uploads\Aws\Exception\UnresolvedEndpointException('This command is set to use ' . 'endpoint discovery, but no endpoint discovery operation was ' . 'found. Please verify the accuracy of your model files.');
        }
        $params = [];
        if (!empty($identifiers)) {
        ...
        $command->getHandlerList()->appendBuild(\UglyRobot\Infinite_Uploads\Aws\Middleware::mapRequest(function (\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $r) {
    ...
    private function handleDiscoveryException($isRequired, $originalUri, \Exception $e, \UglyRobot\Infinite_Uploads\Aws\CommandInterface $cmd, \UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($isRequired) {
            ...
            throw new \UglyRobot\Infinite_Uploads\Aws\Exception\AwsException($message, $cmd, ['code' => 'EndpointDiscoveryException', 'message' => $message], $e);
        }
    ...
    private function modifyRequest(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, $endpoint)
    {
    ...
    private function parseEndpoint($endpoint)
    {
        ...
        throw new \UglyRobot\Infinite_Uploads\Aws\Exception\UnresolvedEndpointException("The supplied endpoint '" . "{$endpoint}' is invalid.");
    }
    private function useOriginalUri(\UglyRobot\Infinite_Uploads\Psr\Http\Message\UriInterface $uri, \UglyRobot\Infinite_Uploads\Aws\CommandInterface $cmd, \UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request)
    {
        $nextHandler = $this->nextHandler;
