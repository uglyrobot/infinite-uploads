...
use UglyRobot\Infinite_Uploads\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \UglyRobot\Infinite_Uploads\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
     ...
     */
    public static function convertPostToGet(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, $additionalQueryParams = "")
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\stream_for(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \UglyRobot\Infinite_Uploads\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \UglyRobot\Infinite_Uploads\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    ...
    private function createPresignedRequest(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, \UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\parse_query($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\build_query($req['query']));
        }
        return new \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
}
