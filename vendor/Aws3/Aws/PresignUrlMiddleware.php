...
use UglyRobot\Infinite_Uploads\Aws\Signature\SignatureV4;
...
class PresignUrlMiddleware
{
    ...
    public function __construct(array $options, callable $endpointProvider, \UglyRobot\Infinite_Uploads\Aws\AwsClientInterface $client, callable $nextHandler)
    {
    ...
    public static function wrap(\UglyRobot\Infinite_Uploads\Aws\AwsClientInterface $client, callable $endpointProvider, array $options = [])
    {
        return function (callable $handler) use($endpointProvider, $client, $options) {
            $f = new \UglyRobot\Infinite_Uploads\Aws\PresignUrlMiddleware($options, $endpointProvider, $client, $handler);
            return $f;
        };
    }
    public function __invoke(\UglyRobot\Infinite_Uploads\Aws\CommandInterface $cmd, \UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function createPresignedUrl(\UglyRobot\Infinite_Uploads\Aws\AwsClientInterface $client, \UglyRobot\Infinite_Uploads\Aws\CommandInterface $cmd)
    {
        $cmdName = $cmd->getName();
        ...
        $request = \UglyRobot\Infinite_Uploads\Aws\serialize($newCmd);
        // Create the new endpoint for the target endpoint.
        $endpoint = \UglyRobot\Infinite_Uploads\Aws\Endpoint\EndpointProvider::resolve($this->endpointProvider, ['region' => $cmd['SourceRegion'], 'service' => $this->serviceName])['endpoint'];
        // Set the request to hit the target endpoint.
        $uri = $request->getUri()->withHost((new \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\Uri($endpoint))->getHost());
        $request = $request->withUri($uri);
        // Create a presigned URL for our generated request.
        $signer = new \UglyRobot\Infinite_Uploads\Aws\Signature\SignatureV4($this->serviceName, $cmd['SourceRegion']);
        ...
        return (string) $signer->presign(\UglyRobot\Infinite_Uploads\Aws\Signature\SignatureV4::convertPostToGet($request, $paramsToAdd ?: ""), $client->getCredentials()->wait(), '+1 hour')->getUri();
    }
}
