...
use UglyRobot\Infinite_Uploads\Aws\Retry\Exception\ConfigurationException;
...
class ConfigurationProvider extends \UglyRobot\Infinite_Uploads\Aws\AbstractConfigurationProvider implements \UglyRobot\Infinite_Uploads\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \UglyRobot\Infinite_Uploads\Aws\Retry\ConfigurationInterface::class;
    protected static $exceptionClass = \UglyRobot\Infinite_Uploads\Aws\Retry\Exception\ConfigurationException::class;
    /**
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($mode)) {
                return \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\promise_for(new \UglyRobot\Infinite_Uploads\Aws\Retry\Configuration($mode, $maxAttempts));
            }
    ...
    {
        return function () {
            return \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\promise_for(new \UglyRobot\Infinite_Uploads\Aws\Retry\Configuration(self::DEFAULT_MODE, self::DEFAULT_MAX_ATTEMPTS));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            return \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\promise_for(new \UglyRobot\Infinite_Uploads\Aws\Retry\Configuration($data[$profile][self::INI_MODE], $maxAttempts));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if (is_int($config)) {
            return new \UglyRobot\Infinite_Uploads\Aws\Retry\Configuration('legacy', $config + 1);
        }
        if (is_array($config) && isset($config['mode'])) {
            $maxAttempts = isset($config['max_attempts']) ? $config['max_attempts'] : self::DEFAULT_MAX_ATTEMPTS;
            return new \UglyRobot\Infinite_Uploads\Aws\Retry\Configuration($config['mode'], $maxAttempts);
        }
