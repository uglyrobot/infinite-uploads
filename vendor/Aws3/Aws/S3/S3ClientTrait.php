...
trait S3ClientTrait
{
    ...
    {
        return (new \UglyRobot\Infinite_Uploads\Aws\S3\ObjectUploader($this, $bucket, $key, $body, $acl, $options))->promise();
    }
    /**
    ...
    public function copyAsync($fromB, $fromK, $destB, $destK, $acl = 'private', array $opts = [])
    {
        ...
        return (new \UglyRobot\Infinite_Uploads\Aws\S3\ObjectCopier($this, $source, $destination, $acl, $opts))->promise();
    }
    /**
    ...
    {
        \UglyRobot\Infinite_Uploads\Aws\S3\StreamWrapper::register($this);
    }
    /**
    ...
    {
        if (!$prefix && !$regex) {
            return new \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException('A prefix or regex is required.'));
        }
        $params = ['Bucket' => $bucket, 'Prefix' => $prefix];
        $iter = $this->getIterator('ListObjects', $params);
        if ($regex) {
            $iter = \UglyRobot\Infinite_Uploads\Aws\filter($iter, function ($c) use($regex) {
                return preg_match($regex, $c['Key']);
            });
        }
        return \UglyRobot\Infinite_Uploads\Aws\S3\BatchDelete::fromIterator($this, $bucket, $iter, $options)->promise();
    }
    /**
    ...
    public function uploadDirectoryAsync($directory, $bucket, $keyPrefix = null, array $options = [])
    {
        ...
        return (new \UglyRobot\Infinite_Uploads\Aws\S3\Transfer($this, $directory, $d, $options))->promise();
    }
    /**
    ...
    public function downloadBucketAsync($directory, $bucket, $keyPrefix = '', array $options = [])
    {
        ...
        return (new \UglyRobot\Infinite_Uploads\Aws\S3\Transfer($this, $s, $directory, $options))->promise();
    }
    /**
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        return $handler($command)->then(static function (\UglyRobot\Infinite_Uploads\Aws\ResultInterface $result) {
            return $result['@metadata']['headers']['x-amz-bucket-region'];
        }, function (\UglyRobot\Infinite_Uploads\Aws\Exception\AwsException $e) {
    ...
    private function determineBucketRegionFromExceptionBody(\UglyRobot\Infinite_Uploads\Psr\Http\Message\ResponseInterface $response)
    {
        try {
