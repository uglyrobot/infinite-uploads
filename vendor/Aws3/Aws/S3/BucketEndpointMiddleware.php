...
class BucketEndpointMiddleware
{
    ...
    private function modifyRequest(\UglyRobot\Infinite_Uploads\Psr\Http\Message\RequestInterface $request, \UglyRobot\Infinite_Uploads\Aws\CommandInterface $command)
    {
        $uri = $request->getUri();
        $path = $uri->getPath();
        $bucket = $command['Bucket'];
        ...
        if ($command['Key']) {
            $path = \UglyRobot\Infinite_Uploads\Aws\S3\S3Client::encodeKey(rawurldecode($path));
        }
