...
use UglyRobot\Infinite_Uploads\Aws\Signature\SignatureTrait;
...
class PostObjectV4
{
     ...
     */
    public function __construct(\UglyRobot\Infinite_Uploads\Aws\S3\S3ClientInterface $client, $bucket, array $formInputs, array $options = [], $expiration = '+1 hours')
    {
        $this->client = $client;
        $this->bucket = $bucket;
        ...
        // setup basic policy
        $policy = ['expiration' => \UglyRobot\Infinite_Uploads\Aws\Api\TimestampShape::format($expiration, 'iso8601'), 'conditions' => $options];
    ...
    {
        $uri = new \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\Uri($this->client->getEndpoint());
    ...
    protected function getPolicyAndSignature(\UglyRobot\Infinite_Uploads\Aws\Credentials\CredentialsInterface $credentials, array $policy)
    {
        $ldt = gmdate(\UglyRobot\Infinite_Uploads\Aws\Signature\SignatureV4::ISO8601_BASIC);
        $sdt = substr($ldt, 0, 8);
