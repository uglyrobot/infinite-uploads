...
use UglyRobot\Infinite_Uploads\Aws\Multipart\AbstractUploader;
...
use UglyRobot\Infinite_Uploads\Psr\Http\Message\StreamInterface as Stream;
use UglyRobot\Infinite_Uploads\Aws\S3\Exception\S3MultipartUploadException;
/**
 * Encapsulates the execution of a multipart upload to S3 or Glacier.
 */
class MultipartUploader extends \UglyRobot\Infinite_Uploads\Aws\Multipart\AbstractUploader
{
     ...
     */
    public function __construct(\UglyRobot\Infinite_Uploads\Aws\S3\S3ClientInterface $client, $source, array $config = [])
    {
        parent::__construct($client, $source, array_change_key_case($config) + ['bucket' => null, 'key' => null, 'exception_class' => \UglyRobot\Infinite_Uploads\Aws\S3\Exception\S3MultipartUploadException::class]);
    }
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = $this->limitPartStream(new \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\LazyOpenStream($this->source->getMetadata('uri'), 'r'));
        } else {
            ...
            $body = \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\stream_for();
            \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\copy_to_stream($source, $body);
        }
    ...
    protected function extractETag(\UglyRobot\Infinite_Uploads\Aws\ResultInterface $result)
    {
        return $result['ETag'];
    }
    protected function getSourceMimeType()
    {
        if ($uri = $this->source->getMetadata('uri')) {
            return \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream';
        }
    }
     ...
     * @return Stream
     */
    private function decorateWithHashes(\UglyRobot\Infinite_Uploads\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        // Decorate source with a hashing stream
        $hash = new \UglyRobot\Infinite_Uploads\Aws\PhpHash('sha256');
        return new \UglyRobot\Infinite_Uploads\Aws\HashingStream($stream, $hash, function ($result) use(&$data) {
