...
use UglyRobot\Infinite_Uploads\Aws\AwsClient;
use UglyRobot\Infinite_Uploads\Aws\CacheInterface;
use UglyRobot\Infinite_Uploads\Aws\ClientResolver;
use UglyRobot\Infinite_Uploads\Aws\Command;
use UglyRobot\Infinite_Uploads\Aws\Exception\AwsException;
use UglyRobot\Infinite_Uploads\Aws\HandlerList;
use UglyRobot\Infinite_Uploads\Aws\InputValidationMiddleware;
use UglyRobot\Infinite_Uploads\Aws\Middleware;
use UglyRobot\Infinite_Uploads\Aws\Retry\QuotaManager;
use UglyRobot\Infinite_Uploads\Aws\RetryMiddleware;
use UglyRobot\Infinite_Uploads\Aws\ResultInterface;
use UglyRobot\Infinite_Uploads\Aws\CommandInterface;
use UglyRobot\Infinite_Uploads\Aws\RetryMiddlewareV2;
use UglyRobot\Infinite_Uploads\Aws\S3\UseArnRegion\Configuration;
use UglyRobot\Infinite_Uploads\Aws\S3\UseArnRegion\ConfigurationInterface;
use UglyRobot\Infinite_Uploads\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use UglyRobot\Infinite_Uploads\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use UglyRobot\Infinite_Uploads\GuzzleHttp\Exception\RequestException;
use UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\Promise;
 ...
 */
class S3Client extends \UglyRobot\Infinite_Uploads\Aws\AwsClient implements \UglyRobot\Infinite_Uploads\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var array */
    ...
    public function __construct(array $args)
    {
        if (!isset($args['s3_us_east_1_regional_endpoint']) || $args['s3_us_east_1_regional_endpoint'] instanceof CacheInterface) {
            $args['s3_us_east_1_regional_endpoint'] = \UglyRobot\Infinite_Uploads\Aws\S3\RegionalEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        parent::__construct($args);
        $stack = $this->getHandlerList();
        $stack->appendInit(\UglyRobot\Infinite_Uploads\Aws\S3\SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 's3.ssec');
        $stack->appendBuild(\UglyRobot\Infinite_Uploads\Aws\S3\ApplyChecksumMiddleware::wrap($this->getApi()), 's3.checksum');
        $stack->appendBuild(\UglyRobot\Infinite_Uploads\Aws\Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type');
        // Use the bucket style middleware when using a "bucket_endpoint" (for cnames)
        if ($this->getConfig('bucket_endpoint')) {
            $stack->appendBuild(\UglyRobot\Infinite_Uploads\Aws\S3\BucketEndpointMiddleware::wrap(), 's3.bucket_endpoint');
        } else {
            $stack->appendBuild(\UglyRobot\Infinite_Uploads\Aws\S3\S3EndpointMiddleware::wrap($this->getRegion(), $this->getConfig('endpoint_provider'), ['dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint')]), 's3.endpoint_middleware');
        }
        $stack->appendBuild(\UglyRobot\Infinite_Uploads\Aws\S3\BucketEndpointArnMiddleware::wrap($this->getApi(), $this->getRegion(), ['use_arn_region' => $this->getConfig('use_arn_region'), 'dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint'), 'endpoint' => isset($args['endpoint']) ? $args['endpoint'] : null]), 's3.bucket_endpoint_arn');
        $stack->appendValidate(\UglyRobot\Infinite_Uploads\Aws\InputValidationMiddleware::wrap($this->getApi(), self::$mandatoryAttributes), 'input_validation_middleware');
        $stack->appendSign(\UglyRobot\Infinite_Uploads\Aws\S3\PutObjectUrlMiddleware::wrap(), 's3.put_object_url');
        $stack->appendSign(\UglyRobot\Infinite_Uploads\Aws\S3\PermanentRedirectMiddleware::wrap(), 's3.permanent_redirect');
        $stack->appendInit(\UglyRobot\Infinite_Uploads\Aws\Middleware::sourceFile($this->getApi()), 's3.source_file');
    ...
    public static function _apply_use_arn_region($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        if ($value instanceof CacheInterface) {
            $value = \UglyRobot\Infinite_Uploads\Aws\S3\UseArnRegion\ConfigurationProvider::defaultProvider($args);
        }
        ...
        if ($value instanceof ConfigurationInterface) {
            ...
            $args['use_arn_region'] = new \UglyRobot\Infinite_Uploads\Aws\S3\UseArnRegion\Configuration($value);
        }
    }
    public function createPresignedRequest(\UglyRobot\Infinite_Uploads\Aws\CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(\UglyRobot\Infinite_Uploads\Aws\serialize($command), $this->getCredentials()->wait(), $expires, $options);
    }
    /**
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \UglyRobot\Infinite_Uploads\Aws\serialize($command)->getUri();
    }
    /**
    ...
    private function getLocationConstraintMiddleware()
    {
        ...
        return static function (callable $handler) use($region) {
            return function (\UglyRobot\Infinite_Uploads\Aws\Command $command, $request = null) use($handler, $region) {
    ...
    private function getSaveAsParameter()
    {
        return static function (callable $handler) {
            return function (\UglyRobot\Infinite_Uploads\Aws\Command $command, $request = null) use($handler) {
    ...
    private function getEncodingTypeMiddleware()
    {
        return static function (callable $handler) {
            return function (\UglyRobot\Infinite_Uploads\Aws\Command $command, $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\UglyRobot\Infinite_Uploads\Aws\ResultInterface $result) use($autoSet) {
    ...
    /** @internal */
    public static function _applyRetryConfig($value, $args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \UglyRobot\Infinite_Uploads\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $maxRetries = $config->getMaxAttempts() - 1;
                $decider = \UglyRobot\Infinite_Uploads\Aws\RetryMiddleware::createDefaultDecider($maxRetries);
                ...
                $delay = [\UglyRobot\Infinite_Uploads\Aws\RetryMiddleware::class, 'exponentialDelay'];
                $list->appendSign(\UglyRobot\Infinite_Uploads\Aws\Middleware::retry($decider, $delay), 'retry');
            } else {
                $defaultDecider = \UglyRobot\Infinite_Uploads\Aws\RetryMiddlewareV2::createDefaultDecider(new \UglyRobot\Infinite_Uploads\Aws\Retry\QuotaManager(), $config->getMaxAttempts());
                $list->appendSign(\UglyRobot\Infinite_Uploads\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'decider' => function ($attempts, \UglyRobot\Infinite_Uploads\Aws\CommandInterface $cmd, $result) use($defaultDecider, $config) {
    ...
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \UglyRobot\Infinite_Uploads\Aws\HandlerList $list)
    {
        \UglyRobot\Infinite_Uploads\Aws\ClientResolver::_apply_api_provider($value, $args);
        $args['parser'] = new \UglyRobot\Infinite_Uploads\Aws\S3\GetBucketLocationParser(new \UglyRobot\Infinite_Uploads\Aws\S3\AmbiguousSuccessParser(new \UglyRobot\Infinite_Uploads\Aws\S3\RetryableMalformedResponseParser($args['parser'], $args['exception_class']), $args['error_parser'], $args['exception_class']));
    }
    /**
     * @internal
     * @codeCoverageIgnore
