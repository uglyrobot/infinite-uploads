...
use UglyRobot\Infinite_Uploads\Aws\MultiRegionClient as BaseClient;
 ...
 */
class S3MultiRegionClient extends \UglyRobot\Infinite_Uploads\Aws\MultiRegionClient implements \UglyRobot\Infinite_Uploads\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var CacheInterface */
    private $cache;
    public static function getArguments()
    {
        ...
        unset($args['region']);
        return $args + ['bucket_region_cache' => ['type' => 'config', 'valid' => [\UglyRobot\Infinite_Uploads\Aws\CacheInterface::class], 'doc' => 'Cache of regions in which given buckets are located.', 'default' => function () {
            return new \UglyRobot\Infinite_Uploads\Aws\LruArrayCache();
        }], 'region' => $regionDef];
    }
    ...
    private function determineRegionMiddleware()
    {
        return function (callable $handler) {
            return function (\UglyRobot\Infinite_Uploads\Aws\CommandInterface $command) use($handler) {
                $cacheKey = $this->getCacheKey($command['Bucket']);
                if (empty($command['@region']) && ($region = $this->cache->get($cacheKey))) {
                    $command['@region'] = $region;
                }
                return \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\coroutine(function () use($handler, $command, $cacheKey) {
                    try {
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        if ($cached = $this->cache->get($cacheKey)) {
            return \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\promise_for($cached);
        }
