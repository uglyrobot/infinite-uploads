...
use UglyRobot\Infinite_Uploads\Aws\Crypto\AbstractCryptoClient;
use UglyRobot\Infinite_Uploads\Aws\Crypto\EncryptionTrait;
 ...
 * @deprecated
 */
class S3EncryptionMultipartUploader extends \UglyRobot\Infinite_Uploads\Aws\S3\MultipartUploader
{
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \UglyRobot\Infinite_Uploads\GuzzleHttp\Promise\promise_for($this->encrypt($this->source, $this->config['@cipheroptions'] ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
