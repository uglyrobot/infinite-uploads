...
trait CryptoParamsTrait
{
    ...
    {
        if (isset($result['Metadata'][\UglyRobot\Infinite_Uploads\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER])) {
            return new \UglyRobot\Infinite_Uploads\Aws\S3\Crypto\HeadersMetadataStrategy();
        }
        return new \UglyRobot\Infinite_Uploads\Aws\S3\Crypto\InstructionFileMetadataStrategy($this->client, $instructionFileSuffix);
    }
        ...
        if (!empty($args['@MetadataStrategy'])) {
            ...
            if (is_string($args['@MetadataStrategy'])) {
                switch ($args['@MetadataStrategy']) {
                    case \UglyRobot\Infinite_Uploads\Aws\S3\Crypto\HeadersMetadataStrategy::class:
                        return new \UglyRobot\Infinite_Uploads\Aws\S3\Crypto\HeadersMetadataStrategy();
                    case \UglyRobot\Infinite_Uploads\Aws\S3\Crypto\InstructionFileMetadataStrategy::class:
                        return new \UglyRobot\Infinite_Uploads\Aws\S3\Crypto\InstructionFileMetadataStrategy($this->client, $instructionFileSuffix);
                    default:
