...
use UglyRobot\Infinite_Uploads\Aws\Multipart\AbstractUploadManager;
use UglyRobot\Infinite_Uploads\Aws\ResultInterface;
use UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7;
class MultipartCopy extends \UglyRobot\Infinite_Uploads\Aws\Multipart\AbstractUploadManager
{
    ...
    {
        if (\UglyRobot\Infinite_Uploads\Aws\Arn\ArnParser::isArn($source)) {
            $this->source = '';
        } else {
            $this->source = "/";
        }
    ...
    private function fetchSourceMetadata()
    {
        ...
        if (strpos($key, '?')) {
            ...
            $headParams['Key'] = $key;
            $query = \UglyRobot\Infinite_Uploads\GuzzleHttp\Psr7\parse_query($query, false);
